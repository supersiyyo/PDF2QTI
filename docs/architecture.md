...
- **Tools**:
    ...
    - **Canvas Critter**: Encapsulated in `/student/canvas-critter`. A dedicated landing page for downloading the 74.4MB standalone desktop application (developed by CSUN students for COMP 380), complete with beta warnings and SiteGround-hosted binary links. Features an integrated **SlidePlayer** component — a dual-mode (inline + theater) cinematic how-to-use guide driven entirely by a `baseUrl` and `slideCount` prop. Images are fetched directly from a SiteGround-hosted folder (`/downloads/slides/<tool-name>/`) and are preloaded on mount for smooth, video-like transitions. The component is fully reusable; new tool guides require only a new SiteGround folder and a single JSX prop change.
