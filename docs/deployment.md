...
6. **Persistence (SQLite)**: Since the Lifeboat is "ephemeral by design," the default transient storage provided by Railway is acceptable. However, for reliability during a live exam, it is recommended to mount a Volume at `/app/database.db` to ensure the SQLite file persists across deployments.
